...
// 闭环控制中间变量
int Vertical_out, Velocity_out, Gyro_Out, Turn_out, Target_Speed =200, Target_turn, MOTO1, MOTO2;
int turn_offset, turn_offset_last;
float Med_Angle = 0; // 平衡时角度值偏移量（机械中值）
// PID参数
float Gyro_Kp = -0.6, Gyro_Ki = -0.42;
float Vertical_Kp = -2.2, Vertical_Kd = -0.02;				  // 直立环
float Velocity_Kp = -0.33, Velocity_Ki = -0;				  // 速度环
float Turn_kp1 , Turn_kp2, Turn_kd1 , Turn_kd2 ; // ppdd转向环
...
{
	float temp;
	temp = turn_offset * Turn_kp1 + turn_offset * turn_offset * Turn_kp2 + (turn_offset - turn_offset_last) * Turn_kd1 + gyro_z * Turn_kd2;
...
void Control(void)
{
	...
	turn_offset =   MT9V03X_W / 2-((Left_Line[TURN_STANDARD] + Right_Line[TURN_STANDARD]) >> 1); // 转向偏差（左正右负）
